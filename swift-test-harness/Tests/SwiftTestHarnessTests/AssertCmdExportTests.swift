...
// of the pipeline:
//
//   1. `embedSwiftExportForXcode` produced `AssertCmd.swiftmodule/`
...
//      `import AssertCmd` above would fail at compile time.
//
//   2. The static archive `libAssertCmd.a` (produced by the
...
//      architecture arm64".
//
//   3. The Kotlin `swiftExport { moduleName = "AssertCmd" }` and
//      `flattenPackage = "io.github.kotlinmania.assertcmd"` configuration in
...
//      via the `AssertCmdLibrary` product.
//
...
final class AssertCmdExportTests: XCTestCase {
    func testSwiftModuleLoads() throws {
        XCTAssertTrue(true, "AssertCmd swift module imported cleanly")
    }
}
