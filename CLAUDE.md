# Claude Code Project Instructions — assert-cmd-kotlin

## Project Overview

This is **assert-cmd-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`assert_cmd`](https://crates.io/crates/assert_cmd).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/assert_cmd/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/assert_cmd>
