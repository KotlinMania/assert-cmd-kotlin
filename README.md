...
Kotlin Multiplatform line-by-line clean-room port of the Rust crate [`assert_cmd`](https://crates.io/crates/assert_cmd).

> **Status: scaffold — porting has not started.** This repo currently contains build infrastructure only. The upstream Rust source for the `assert_cmd` crate will be cloned into `tmp/` (gitignored) when porting begins.

## About

Assertions for testing CLI binaries
...
Original work copyrighted by the upstream `assert_cmd` authors. Kotlin port copyright (c) 2026 Sydney Renee and The Solace Project.
